...
void XPlayBuf::next(int nSamples) {
  ...
  for (int32 outSample = 0; outSample < nSamples; ++outSample) {
    // not looping and out of bounds: clear remaining out samples, release buf
    // and exit
    ...
    // update currLoop bounds: if loop args changed w/o trig, update currLoop
    // when fade is small
...
void XPlayBuf::loadLoopArgs() {
  m_currLoop.phase =
      sc_mod(static_cast<double>(m_argLoopStart) * m_buf->samplerate,
             static_cast<double>(m_bufFrames));
  ...
  if (m_argLoopDur < 0) {
    ...
    m_currLoop.end =
        sc_mod(static_cast<int32>(m_currLoop.phase +
                                  static_cast<double>(m_argLoopDur) *
                                      m_buf->samplerate),
               m_bufFrames);
  }
...
// return true if loop args changed w/o a trig, to signal that currLoop needs to
// be updated in ::next()
void XPlayBuf::readInputs() {
  m_playbackRate = static_cast<double>(in0(UGenInput::playbackRate)) *
                   m_buf->samplerate * sampleDur();
  m_isLooping = static_cast<bool>(in0(UGenInput::looping));

  int argFadeSamples = static_cast<int32>(
      sc_floor(in0(UGenInput::fadeTime) * m_buf->samplerate + .5));
  if (argFadeSamples != m_totalFadeSamples) {
    // calc reciprocal only on change
    m_totalFadeSamples = argFadeSamples;
    m_oneOverFadeSamples =
        m_totalFadeSamples > 0
            ? sc_reciprocal(static_cast<float>(m_totalFadeSamples))
            : 1.;
  }

  float argLoopStart = in0(UGenInput::startPos);
  float argLoopDur = in0(UGenInput::loopDur);
  m_loopChanged = (m_loopChanged || argLoopStart != m_argLoopStart ||
  ...
  if (triggered) {  // start cross-fade: copy old loop to prevLoop, set
                    // m_remainingXFadeSamples
    mDone = false;
    m_prevLoop = m_currLoop;
    loadLoopArgs();
    m_remainingXFadeSamples = static_cast<int32>(
        sc_floor(in0(UGenInput::xFadeTime) * m_buf->samplerate + .5));
    if (m_remainingXFadeSamples < 0)
      m_remainingXFadeSamples = m_totalFadeSamples;
    m_oneOverXFadeSamples =
        m_remainingXFadeSamples > 0
            ? sc_reciprocal(static_cast<float>(m_remainingXFadeSamples))
            : 1.;

    m_loopChanged =
        false;  // currLoop was already updated: no need to signal ::next()
...
void XPlayBuf::writeFrame(int32 outSample) {
  ...
  for (uint32 ch = 0; ch < m_numWriteChannels; ++ch) {
    out(ch)[outSample] =
        cubicinterp(fracphase, s0[ch], s1[ch], s2[ch], s3[ch]) *
        m_currLoop.fade;
  }
}
...
void XPlayBuf::xfadeFrame(int32 outSample) {
  ...
  for (uint32 ch = 0; ch < m_numWriteChannels; ++ch) {
    out(ch)[outSample] = xfade_equalPower(
        cubicinterp(fracphase, s0[ch], s1[ch], s2[ch], s3[ch]) *
            m_currLoop.fade,
        cubicinterp(prev_fracphase, prev_s0[ch], prev_s1[ch], prev_s2[ch],
                    prev_s3[ch]) *
            m_prevLoop.fade,
        xfade);
  }
...
// wrap pos in loop and buf bounds, and compute current fade (relative to
// bounds)
...
bool XPlayBuf::isLoopPosOutOfBounds(const Loop& loop,
                                    const int32 iphase) const {
  if (loop.isEndGTStart) {
    return iphase < loop.start || iphase > loop.end;
  }
  return (iphase < loop.start && iphase > loop.end) ||
         (iphase < 0 || iphase > m_bufFrames);
}
...
int32 XPlayBuf::wrapPos(int32 iphase, const Loop& loop) const {
  if (isLoopPosOutOfBounds(loop, iphase)) {
    if (loop.isEndGTStart) {
      ...
      // loop.end < loop.start
      // needs to choose one of four wrap strategies, depending on pos and
      // forward vs. reverse
      if (iphase > m_bufFrames) {
        ...
        if (iphase - m_bufFrames < loop.end) {
          ...
          if (iphase > m_bufFrames) iphase -= m_bufFrames;
        }
      } else if (iphase < 0) {
        ...
        iphase = loop.start +
                 sc_mod(iphase - loop.end, loop.end + m_bufFrames - loop.start);
        if (iphase > m_bufFrames) iphase -= m_bufFrames;
      } else {
        // end < iphase < start: backwards
        iphase = loop.end - sc_mod(loop.start - iphase,
                                   loop.end + m_bufFrames - loop.start);
        if (iphase < 0) iphase += m_bufFrames;
      };
    }
  }
  return iphase;
...
// depends on proximity to loop bounds, and also to buf bounds if loop spans
// across them (start > end)
...
float XPlayBuf::xfade_equalPower(float fadingIn, float fadingOut,
                                 double fade) const {
...
bool XPlayBuf::getBuf(int nSamples) {
  ...
  if (!m_buf->data) {
    ...
    if (m_buf->channels != mNumOutputs) {
      if (mWorld->mVerbosity > -1 && !mDone && (m_failedBufNum != fbufnum)) {
        Print(
            "Buffer UGen channel mismatch: expected %i, yet buffer has %i "
            "channels\n",
